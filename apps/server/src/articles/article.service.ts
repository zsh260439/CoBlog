...
export class ArticlesService {
  ...
  async findBySlug(slug: string) {
    ...
    const [prev, next] = await Promise.all([
      this.articleModel.findOne({ createdAt: { $lt: article.createdAt } }, { slug: 1, title: 1 }).sort({ createdAt: -1 }).lean(),
      this.articleModel.findOne({ createdAt: { $gt: article.createdAt } }, { slug: 1, title: 1 }).sort({ createdAt: 1 }).lean(),
    ])

    const related = await this.articleModel
      .find({ categorySlug: article.categorySlug, slug: { $ne: slug } }, { slug: 1, title: 1, coverImage: 1, excerpt: 1, createdAt: 1 })
      .sort({ createdAt: -1 })
      .limit(6)
      .lean()
    ...
    return {
      ...
      related: related.map((item: Article) => this.serializeArticle(item)),
    }
  }
